...
class LivePlotterDockWidget(QDockWidget):
    ...
    def new_plot(self, dist_computer: DistanceComputer, feed_fps: int):
        ...
        self._mutex.acquire()  # because of frame number attribute
        LivePlotterDockWidget.plot_new_point(plot_widget, dist_computer.distance(), self._current_frame_number, update=False)
        self._mutex.release()
    ...
    def update_plots(self, frame_number: int):
        ...
        for (dist_computer, plot_widget) in self._plot_widgets.items():
            ...
            if distance != DistanceComputer.ERR_DIST:
                LivePlotterDockWidget.plot_new_point(plot_widget, distance, self._current_frame_number, update=True)
        self._mutex.release()
    ...
    @staticmethod
    def _x_time_computation(fps: int, frame_number: int) -> float:
        return frame_number / fps

    @classmethod
    def plot_new_point(cls, plot_widget: DistancePlotWidget, distance: float, current_frame_number: int,
                       update: bool = False):
        feed_fps = plot_widget.get_fps()
        new_x = LivePlotterDockWidget._x_time_computation(feed_fps, current_frame_number)
        new_y = distance
